...
```{r}
df_daa_prf_pts <- df_daa_prf_pts %>%
  mutate(PTS_max_total = case_when(DAA == "DCV/ASV" ~ TOTAL/(168*2),
...
# age adjustement based on all

```{r}
df_daa_prf_ann <- df_daa_prf_pts %>%
  distinct(SHEET, YEAR_str, DAA, .keep_all = TRUE) %>%
  group_by(YEAR_str) %>%
  summarise(SUM = sum(PTS_max_total))

df_daa_prf_ann
```

```{r}
df_pop_total_2015 <- df_pop_prf %>% 
  filter(PRF_jp == "total") %>%
  rename(POP = "ALL_total")
```

```{r}
pop_total_2015 <- df_pop_total_2015 %>%
  filter(YEAR == "2015") %>%
  pull(POP)
...
```{r}
df_daa_prf_ann <- df_daa_prf_ann %>%
  rename(YEAR = "YEAR_str") %>%
  left_join(df_pop_total_2015 %>%
              select(YEAR, POP),
            by = join_by(YEAR))
...
```{r}
df_daa_prf_ann %>%
  mutate(POP_2015 = 127095) %>%
  mutate(SUM_adj = SUM * POP_2015 / POP) %>%
  summarise(SUM_adj = sum(SUM_adj) )
```

```{r}
df_daa_prf_ann %>%
  mutate(POP_2015 = pop_total_2015) %>%
  mutate(SUM_adj = SUM * POP_2015 / POP) %>%
  summarise(SUM_adj = sum(SUM_adj) ) %>%
  mutate(PCT_adj = SUM_adj / (pop_total_2015 * 10^3 )*100  )
```
```{r}
pct_mean_total <- df_daa_prf_ann %>%
  mutate(POP_2015 = pop_total_2015) %>%
  mutate(SUM_adj = SUM * POP_2015 / POP) %>%
  summarise(SUM_adj = sum(SUM_adj) ) %>%
  mutate(PCT_adj = SUM_adj / (pop_total_2015 * 10^3 )*100 ) %>%
...
# use this adjusted prevalence based on national data
# rahter than sum of geographic data
```


# national adjustment based on prefecture
...
p1_a2 <- df_daa_prf_pts_adj %>%
  ...
  geom_col(width = 0.7, alpha = 1)+
  geom_hline(yintercept = pct_mean_total,
...
```{r}
prf_11 <- df_daa_prf_pts_adj %>%
  ungroup() %>%
  slice_max(SUM_adj, n = 11) %>%
  pull(PRF)

prf_filter <- c(prf_11, "Miyagi", "Okinawa") 
```


```{r}
df_daa_prf_pts_name <- df_daa_prf_pts_adj %>%
   mutate(ALL_2015 = ALL_2015/1000) %>%
  filter(PRF  %in% prf_filter) %>%
  mutate(hjust = case_when(PRF %in% c("Aichi", "Shizuoka",
                                      "Miyagi", "Okinawa",
                                      "Aichi") ~ 0,
                           PRF %in% c( "Chiba", "Hyogo") ~ 1,
                           .default = 0.5)) %>%
  mutate(vjust = case_when(PRF %in% c("Hokkaido",
                       ...
                       PRF %in% c("Miyagi",
                                  ...
                                  "Aichi") ~ 0.5,
                       .default = 0)) %>%
  mutate(x = case_when(PRF == "Shizuoka" ~ ALL_2015,
                       hjust == 1 ~ ALL_2015 - 15/40,
                       hjust == 0 ~ ALL_2015 + 15/40,
                       .default = ALL_2015)) %>%
  mutate(y = case_when(vjust == 1 ~ SUM_adj - 30000/20,
                       vjust == 0 ~ SUM_adj + 30000/20,
                       .default = SUM_adj)) 
```
...
```{r}
saveRDS(df_daa_prf_pts, "../df/daa/analysis/df_daa_prf_pts")
```
