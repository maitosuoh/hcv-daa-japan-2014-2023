...
```{r}
# remove dcv ebr
df_daa_total_pts <- df_daa_total_imp %>%
  mutate(DAA_ind = DAA) %>%
...
df_daa_total_pts <- df_daa_total_pts %>%
...
```{r}
df_daa_dem_pts_all <- df_daa_total_pts %>%
  group_by(YEAR_str) %>%
  summarise(SUM = sum(PTS_max)) %>%
  mutate(DAA = "All")
